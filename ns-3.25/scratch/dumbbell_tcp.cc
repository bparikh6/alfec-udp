...
NS_LOG_COMPONENT_DEFINE("Four_Tcp");
...
main (int argc, char *argv[]){
 ...
 //Config::SetDefault("ns3::TcpSocket::SegmentSize", UintegerValue(1448));
  ...
  //create packet sink install on 2 
   ...
   uint16_t sinkPort2 = 8800;
   BulkSendHelper bulkSend2 ("ns3::TcpSocketFactory", InetSocketAddress (i10i7.GetAddress (0), sinkPort2));
   ...
   //create packet sink install on 10
   PacketSinkHelper packetSinkHelper2 ("ns3::TcpSocketFactory", InetSocketAddress(Ipv4Address::GetAny(), sinkPort2));
   ApplicationContainer sinkApps2 = packetSinkHelper2.Install(c.Get (10));
   sinkApps2.Start(Seconds(0.0));
   
   
   //create bulk send on 9
   uint16_t sinkPort3 = 10;
  BulkSendHelper bulkSend3 ("ns3::TcpSocketFactory", InetSocketAddress (i11i7.GetAddress (0), sinkPort3));
   bulkSend3.SetAttribute("MaxBytes", UintegerValue (maxBytes));
    bulkSend3.SetAttribute("SendSize", UintegerValue (1000));
  ApplicationContainer sourceApps3 = bulkSend3.Install(c.Get(9));
   sourceApps3.Start(Seconds(0.0));
 
  //create packet sink install on 11 
   PacketSinkHelper packetSinkHelper3 ("ns3::TcpSocketFactory", InetSocketAddress(Ipv4Address::GetAny(), sinkPort3));
   ApplicationContainer sinkApps3 = packetSinkHelper3.Install(c.Get (11));
  ...
  Simulator::Schedule(Seconds(1000), &Throughput);  

  AsciiTraceHelper ascii;
  p2p.EnableAsciiAll (ascii.CreateFileStream ("tcp_pair.tr"));
  p2p.EnablePcapAll ("tcp_4pair", true);
