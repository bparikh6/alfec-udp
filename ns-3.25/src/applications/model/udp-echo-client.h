...
private:
  ...
  /*
   * Divides the whole object into blocks of source symbols based upon the 
   * block number that is passed as a parameter from the function Send()

   * returns the block of data in the form of special data structure 
   * Array_Data_Symbol defined Raptor Codes library
  */

  Array_Data_Symbol divideIntoBlocks(uint16_t Z);


  /*
   * When passed an independent block's source symbols to this function, it encodes
   * source block by adding defined overhead.
   *
   * returns encoded symbols of a block
  */
  Array_Data_Symbol Raptor_Encoding (Array_Data_Symbol T);

  /*
   * This function breaks down a blocks encoded symbols to number of symbols
   * that each packet can carry with it (here 1000)

   * returns the chunk of encoded symbols that fits into a packet
  */

  std::vector<uint8_t> dataToSendInEachPacket(Array_Data_Symbol S, uint32_t K, double G);

  void sendBack(Ptr<Socket> soc, Ptr<Packet> packet);
  Array_Data_Symbol encodedSymbols;

  std::list<Array_Data_Symbol> m_block;
  
  std::vector<uint8_t> packetSymbol;
